 ...
 * 
 * <p/>
 * This implementation uses internally an {@link ArtifactLocator} to retrieve
 * the required dependencies for the running test. By default, the artifact
 * locator uses the local maven 2 repository. Maven configurations (such as
 * &lt;settings.xml&gt;) are supported. Alternatively for Maven repositories
 * located in non-default locations, one can use the
...
public abstract class AbstractDependencyManagerTests extends AbstractSynchronizedOsgiTests {
     ...
     * 
     * <p/>
     * A bundle name can have any value and depends on the format expected by
     * the {@link ArtifactLocator} implementation. By default, a CSV (Comma
     * Separated Values) format is expected.
     * 
     * <p/>
     * This method allows a declarative approach in declaring bundles as opposed
     * to {@link #getTestBundles()} which provides a programmatic one.
     * 
     ...
     * 
     * <p/>
     * This implementation reads a predefined properties file to determine the
     * bundles needed. If the configuration needs to be changed, consider
     ...
     * 
     * <p/>
     * Sets specific log4j property to avoid class loading problems during start
     * up related to the thread context class loader.
     */
