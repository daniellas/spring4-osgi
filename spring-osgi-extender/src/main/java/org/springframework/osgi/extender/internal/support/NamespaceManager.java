...
public class NamespaceManager implements InitializingBean, DisposableBean {
    ...
    public void maybeAddNamespaceHandlerFor(Bundle bundle) {
        ...
        if (hasHandlers) {
            // check type compatibility between the bundle's and
            // spring-extender's spring version
            ...
            // bundle declares only schemas, add it though the handlers might
            // not be compatible...
            if (hasSchemas)
                addHandler(bundle);
        }
    ...
    private void registerResolverServices() {
        ...
        if (bnd != null) {
            ...
            // props.setProperty(BundleUtils.DM_CORE_ID, "" +
            // bnd.getBundleId());
            // props.setProperty(BundleUtils.DM_CORE_TS, "" +
            // bnd.getLastModified());
        }
